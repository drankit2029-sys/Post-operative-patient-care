...
async def evaluate_missed_reminder(
    ...
    if not openai_client or not settings.OPENAI_API_KEY:
        logger.info("OpenAI API key unset; utilizing heuristic engine for reminder evaluation.")
...
1. Assess the risk of missing this specific dose in the context of the primary diagnosis and hospital course.
   - 'critical': Immediately life-threatening if missed (e.g., dual antiplatelet therapy after PCI/stent, essential insulin, critical antiarrhythmics).
   - 'high': Serious risk of rapid clinical deterioration (e.g., core antihypertensives in hypertensive emergency, heart failure loop diuretics, critical anticoagulants).
   - 'medium': Standard antibiotics, oral steroids, maintenance inhalers, where missing one dose requires clinical follow-up.
   - 'low': General wellness reminders, non-urgent vitamins, routine mobility checks.
...
"""
    try:
        response = await openai_client.beta.chat.completions.parse(
            ...
            messages=[
                {"role": "system", "content": "You are a clinical triage AI that evaluates missed patient care regimens."},
        ...
        logger.error(f"OpenAI reminder evaluation failed: {e}. Falling back to heuristic.")
        return heuristic_missed_reminder(patient_summary, medication_info, scheduled_time)
...
async def evaluate_monitor_telemetry(
    ...
    if not openai_client or not settings.OPENAI_API_KEY:
        logger.info("OpenAI API key unset; utilizing heuristic engine for telemetry evaluation.")
    ...
    system_instruction = (
        "You are an expert clinical vision diagnostic assistant. "
        "Analyze patient telemetry photos/videos, check against alert trigger rules, "
...
"""
    ...
    if base64_image:
        ...
        messages.append({
            "role": "user",
            "content": context_prompt + "\n[Note: Media file was not provided or could not be decoded. Evaluate based on textual report and history.]"
        })
    ...
    try:
        ...
        logger.error(f"OpenAI monitor evaluation failed: {e}. Falling back to heuristic.")
        return heuristic_monitor_telemetry(patient_summary, monitor_spec, past_remarks, user_notes)
